...
@implementation SQCConfiguration

#pragma mark - Singleton Methods

+ (instancetype) sharedConfig {
    static SQCConfiguration *shardConfig = nil;
...
- (instancetype) init {
    ...
    if (self) {
        self.apiClient = nil;
        self.host = @"https://connect.squareup.com";
        self.username = @"";
        self.password = @"";
        self.accessToken= @"";
        self.verifySSL = YES;
        ...
        self.mutableDefaultHeaders[@"User-Agent"] = [NSString stringWithFormat:@"Swagger-Codegen/2.0/objc (%@; iOS %@; Scale/%0.2f)",[[UIDevice currentDevice] model], [[UIDevice currentDevice] systemVersion], [[UIScreen mainScreen] scale]];
        self.logger = [SQCLogger sharedLogger];
    }
    return self;
}
