...
#ifndef SQCDebugLogResponse
#define SQCDebugLogResponse(response, responseObject,request, error) [[SQCLogger sharedLogger] logResponse:response responseObject:responseObject request:request error:error];
#endif
 ...
 */
#ifndef SQCDebugLog
#define SQCDebugLog(format, ...) [[SQCLogger sharedLogger] debugLog:[NSString stringWithFormat:@"%s", __PRETTY_FUNCTION__] message: format, ##__VA_ARGS__];
#endif

@interface SQCLogger : NSObject

+(instancetype)sharedLogger;

/**
 * Enabled switch, default NO - default set by SQCConfiguration debug property
 */
