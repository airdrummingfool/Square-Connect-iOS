#import "SQCApiClient.h"

NSString *const SQCResponseObjectErrorKey = @"SQCResponseObject";
...
static NSDictionary * SQC__headerFieldsForResponse(NSURLResponse *response) {
...
static NSString * SQC__fileNameForResponse(NSURLResponse *response) {
    NSDictionary * headers = SQC__headerFieldsForResponse(response);
...
@interface SQCApiClient ()

@property (nonatomic, strong) NSDictionary* HTTPResponseHeaders;

@end

@implementation SQCApiClient

- (instancetype)init {
    NSString *baseUrl = [[SQCConfiguration sharedConfig] host];
...
- (instancetype)initWithBaseURL:(NSURL *)url {
    ...
    if (self) {
        ...
        self.responseDeserializer = [[SQCResponseDeserializer alloc] init];
        self.sanitizer = [[SQCSanitizer alloc] init];
...
+(NSNumber*) queueRequest {
    ...
    SQCDebugLog(@"added %@ to request queue", requestId);
...
-(Boolean) executeRequestWithId:(NSNumber*) requestId {
    ...
    if (matchingItems.count == 1) {
        SQCDebugLog(@"removed request id %@", requestId);
        [queuedRequests removeObject:requestId];
        return YES;
    } else {
        return NO;
...
- (void) configureCacheReachibility {
    [self.reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        reachabilityStatus = status;
        SQCDebugLog(@"reachability changed to %@",AFStringFromNetworkReachabilityStatus(status));
        [SQCApiClient setOfflineState:status == AFNetworkReachabilityStatusUnknown || status == AFNetworkReachabilityStatusNotReachable];
...
- (void) operationWithCompletionBlock: (NSURLRequest *)request
    ...
    NSURLSessionDataTask* op = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        SQCDebugLogResponse(response, responseObject,request,error);
        strongSelf.HTTPResponseHeaders = SQC__headerFieldsForResponse(response);
        if(!error) {
        ...
        if (responseObject) {
            ...
            userInfo[SQCResponseObjectErrorKey] = responseObject;
        }
...
- (void) downloadOperationWithCompletionBlock: (NSURLRequest *)request
    ...
    NSURLSessionDataTask* op = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        NSString * filename = SQC__fileNameForResponse(response);
...
-(NSNumber*) requestWithPath: (NSString*) path
    ...
    if ([requestContentType isEqualToString:@"application/json"]) {
        self.requestSerializer = [SQCJSONRequestSerializer serializer];
    }
    ...
    if ([responseContentType isEqualToString:@"application/json"]) {
        self.responseSerializer = [SQCJSONResponseSerializer serializer];
    } else {
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        safeString = SQCPercentEscapedStringFromString(safeString);
    ...
    if (offlineState) {
        SQCDebugLog(@"%@ cache forced", resourcePath);
        [request setCachePolicy:NSURLRequestReturnCacheDataDontLoad];
    }
    else if(!hasHeaderParams && [method isEqualToString:@"GET"] && cacheEnabled) {
        SQCDebugLog(@"%@ cache enabled", resourcePath);
        [request setCachePolicy:NSURLRequestUseProtocolCachePolicy];
    }
    else {
        SQCDebugLog(@"%@ cache disabled", resourcePath);
    ...
    NSNumber* requestId = [SQCApiClient queueRequest];
...
- (NSString*) pathWithQueryParamsToString:(NSString*) path
    ...
    for(NSString * key in [queryParams keyEnumerator]){
        ...
        NSString *safeKey = SQCPercentEscapedStringFromString(key);
        if ([queryParam isKindOfClass:[NSString class]]){
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, SQCPercentEscapedStringFromString(queryParam)]];

        } else if ([queryParam isKindOfClass:[SQCQueryParamCollection class]]){
            SQCQueryParamCollection * coll = (SQCQueryParamCollection*) queryParam;
            ...
            if([format isEqualToString:@"multi"]) {
                for(id obj in values) {
                    ...
                    NSString * safeValue = SQCPercentEscapedStringFromString([NSString stringWithFormat:@"%@",obj]);
            ...
            NSString * safeValue = SQCPercentEscapedStringFromString([values componentsJoinedByString:separatorStyle]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        } else {
            NSString * safeValue = SQCPercentEscapedStringFromString([NSString stringWithFormat:@"%@",queryParam]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        }
        counter += 1;
    }
...
- (AFSecurityPolicy *) customSecurityPolicy {
    ...
    SQCConfiguration *config = [self configuration];
...
- (SQCConfiguration*) configuration {
    return [SQCConfiguration sharedConfig];
}
