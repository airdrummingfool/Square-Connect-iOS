...
NSString *const SCResponseObjectErrorKey = @"SCResponseObject";
...
static NSDictionary * SC__headerFieldsForResponse(NSURLResponse *response) {
...
static NSString * SC__fileNameForResponse(NSURLResponse *response) {
    NSDictionary * headers = SC__headerFieldsForResponse(response);
...
- (instancetype)init {
    NSString *baseUrl = [[SCConfiguration sharedConfig] host];
...
- (instancetype)initWithBaseURL:(NSURL *)url {
    ...
    if (self) {
        ...
        self.responseDeserializer = [[SCResponseDeserializer alloc] init];
        self.sanitizer = [[SCSanitizer alloc] init];
...
+(NSNumber*) queueRequest {
    ...
    SCDebugLog(@"added %@ to request queue", requestId);
...
-(Boolean) executeRequestWithId:(NSNumber*) requestId {
    ...
    if (matchingItems.count == 1) {
        SCDebugLog(@"removed request id %@", requestId);
        [queuedRequests removeObject:requestId];
        return YES;
    } else {
        return NO;
...
- (void) configureCacheReachibility {
    [self.reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        reachabilityStatus = status;
        SCDebugLog(@"reachability changed to %@",AFStringFromNetworkReachabilityStatus(status));
        [SCApiClient setOfflineState:status == AFNetworkReachabilityStatusUnknown || status == AFNetworkReachabilityStatusNotReachable];
...
- (void) operationWithCompletionBlock: (NSURLRequest *)request
    ...
    NSURLSessionDataTask* op = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        SCDebugLogResponse(response, responseObject,request,error);
        strongSelf.HTTPResponseHeaders = SC__headerFieldsForResponse(response);
        if(!error) {
        ...
        if (responseObject) {
            ...
            userInfo[SCResponseObjectErrorKey] = responseObject;
        }
...
- (void) downloadOperationWithCompletionBlock: (NSURLRequest *)request
    ...
    NSURLSessionDataTask* op = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        NSString * filename = SC__fileNameForResponse(response);
...
-(NSNumber*) requestWithPath: (NSString*) path
    ...
    if ([requestContentType isEqualToString:@"application/json"]) {
        self.requestSerializer = [SCJSONRequestSerializer serializer];
    }
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        safeString = SCPercentEscapedStringFromString(safeString);
    ...
    if (offlineState) {
        SCDebugLog(@"%@ cache forced", resourcePath);
        [request setCachePolicy:NSURLRequestReturnCacheDataDontLoad];
    }
    else if(!hasHeaderParams && [method isEqualToString:@"GET"] && cacheEnabled) {
        SCDebugLog(@"%@ cache enabled", resourcePath);
        [request setCachePolicy:NSURLRequestUseProtocolCachePolicy];
    }
    else {
        SCDebugLog(@"%@ cache disabled", resourcePath);
    ...
    NSNumber* requestId = [SCApiClient queueRequest];
...
- (NSString*) pathWithQueryParamsToString:(NSString*) path
    ...
    for(NSString * key in [queryParams keyEnumerator]){
        ...
        NSString *safeKey = SCPercentEscapedStringFromString(key);
        if ([queryParam isKindOfClass:[NSString class]]){
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, SCPercentEscapedStringFromString(queryParam)]];

        } else if ([queryParam isKindOfClass:[SCQueryParamCollection class]]){
            SCQueryParamCollection * coll = (SCQueryParamCollection*) queryParam;
            ...
            if([format isEqualToString:@"multi"]) {
                for(id obj in values) {
                    ...
                    NSString * safeValue = SCPercentEscapedStringFromString([NSString stringWithFormat:@"%@",obj]);
            ...
            NSString * safeValue = SCPercentEscapedStringFromString([values componentsJoinedByString:separatorStyle]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        } else {
            NSString * safeValue = SCPercentEscapedStringFromString([NSString stringWithFormat:@"%@",queryParam]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        }
        counter += 1;
    }
...
- (AFSecurityPolicy *) customSecurityPolicy {
    ...
    SCConfiguration *config = [self configuration];
...
- (SCConfiguration*) configuration {
    return [SCConfiguration sharedConfig];
}
